...
sed -i -e '/uci commit firewall/i \\tuci set firewall.@defaults[0].flow_offloading=1' friendlywrt/target/linux/rockchip/armv8/base-files/root/setup.sh
sed -i -e '/uci commit firewall/i \\tuci set firewall.@defaults[0].flow_offloading_hw=1' friendlywrt/target/linux/rockchip/armv8/base-files/root/setup.sh
